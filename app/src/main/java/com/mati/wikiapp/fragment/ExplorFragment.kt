...
import com.mati.wikiapp.MainActivity2
import com.mati.wikiapp.adapter.ExplorAdapter
import com.mati.wikiapp.adapter.ItemEvent
import com.mati.wikiapp.data.item_post
import com.mati.wikiapp.databinding.FragmentExplorBinding

const val SEND_DATA_ACTIVITY = "sendData"

class ExplorFragment : Fragment(), ItemEvent {

    lateinit var binding: FragmentExplorBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        binding = FragmentExplorBinding.inflate(layoutInflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val myAdapter = ExplorAdapter(dataExplor, this)

        binding.viewExprlor.layoutManager =
            LinearLayoutManager(context, RecyclerView.VERTICAL, false)
        binding.viewExprlor.adapter = myAdapter
    }

    override fun onItemClicked(itemPost: item_post) {
        val intent = Intent(activity, MainActivity2::class.java)
        intent.putExtra(SEND_DATA_ACTIVITY, itemPost)
